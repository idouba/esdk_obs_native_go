...
//?? Reconcile this with parse()
    ...
    public CIMNameSpace(String host, String ns) {
    	...
    	/*  T11V-2616  使用IPV6发现设备T系列V1R3，无法创建RAID组和LUN  c00001507 begin*/
        //Begin xiekaiji 安全红线支持HTTPS
//        this((!host.startsWith("https://") && host.contains(":") && !host.contains("[")) ? "[" + host + "]"
//                : host);
        this(getHost(host));
        //End xiekaiji 安全红线支持HTTPS
		/*  T11V-2616  使用IPV6发现设备T系列V1R3，无法创建RAID组和LUN  c00001507 end*/
		nameSpace = validateNameSpace(ns);
    }
    
    private static String getHost(String host)
    {
        StringBuffer returnHost = new StringBuffer();
        if (host.startsWith("https://"))
        {
            //截取IP
            String hostIp = host.substring(8);
            //IPv6
            if (hostIp.contains(":"))
            {
                returnHost.append("https://");
                returnHost.append("[");
                returnHost.append(hostIp);
                returnHost.append("]");
            }
            //IPv4
            else
            {
                returnHost.append(host);
            }
        }
        else
        {
    ...
    {
        if (null != host && host.contains(":") && !host.contains("["))
        {
            return "[" + host + "]";
        }
        return host;
    ...
    public String toString() {
        ...
        final boolean haveHost = host.length() > 0;
        final boolean haveNameSpace = nameSpace.length() > 0;
