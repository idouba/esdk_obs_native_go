...
final class ServerInfo implements Cloneable
{
    ...
    /**
     * Sets authentication information based on parameters contained in given
     * challenge string (which should be the value of either the
     * "WWW-Authenticate" or "Proxy-Authenticate" header fields).  If given
     * string is null or empty, clears any previous authentication information.
     */
    void setAuthInfo(String str) throws HttpParseException
    {
        ...
        //Workaround for SNIA & Pegasus CIMOM
        //If the Auth entry is missing the realm= we 
        //will do a best guess fix
        if (str.indexOf("realm") == -1)
        {
            ...
            while (parser.hasMoreTokens())
            {
                ...
                if (!token.startsWith("Basic") && !token.startsWith("Digest"))
                {
                    buf.append("realm=");
                }
                buf.append(token + " ");
        ...
        if ((entries = lp.getEntries("Digest")) != null)
        {
    ...
    /**
     * Updates authentication information based on parameters contained in
     * given string (which should be the value of either the
     * "Authorization-Info" or "Proxy-Authorization-Info" header fields).  If
     * given string is null or empty, current authentication settings are
     * left unchanged.
     */
    void updateAuthInfo(String str) throws HttpParseException
    {
        ...
        {
            LineParser lp = new LineParser(str);
            HashMap entries = lp.getAllEntries();
    ...
    String getAuthString(String protocol, String method, String uri)
    {
        ...
        if (authUser == null)
        {
            ...
            char[] passwd = pa.getPassword();
            if ((user == null) || (passwd == null))
            {
                return null;
            }
            authUser = user;
            authPassword = new String(passwd);
        }
        ...
        else if (authScheme.equals("Digest"))
        {
            ...
            String response = "Digest " + "username=\"" + authUser + "\", "
                    + "realm=\"" + authRealm + "\", " + "nonce=\"" + authNonce
            ...
            throws NoSuchAlgorithmException
    {
        ...
        MessageDigest md = MessageDigest.getInstance((authAlgorithm != null) ? authAlgorithm
                : "MD5");
        String hashA1 = encode(md, authUser + ":" + authRealm + ":"
                + authPassword);
    ...
    private static final class LineParser
    {
        ...
        {
            HashMap map = null;
            String[][] ea = (String[][]) entries.toArray(new String[entries.size()][]);
    ...
    private PasswordAuthentication getPassword(final String protocol)
    {
        return (PasswordAuthentication) AccessController.doPrivileged(new PrivilegedAction()
        {
            public Object run()
            {
                ...
                return CIMAuthenticator.requestPasswordAuthentication(addr,
                        port,
                        protocol,
                        authRealm,
                        authScheme);
