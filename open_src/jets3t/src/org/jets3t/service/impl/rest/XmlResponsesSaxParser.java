...
//    }
            ...
            else if (name.equals("LastModified"))
            {
                ...
                catch (ParseException e)
                {
                    throw new RuntimeException(
                        "Non-ISO8601 date for LastModified in bucket's versions listing output: " + elementText, e);
                }
            }
            else if (name.equals("ETag"))
            {
    ...
    public class CompleteMultipartUploadErrorHandler extends SimpleHandler
    {
        ...
        {
            String fullMessage =
                message + ": PartNumber=" + partNumber + ", MinSizeAllowed=" + minSizeAllowed + ", ProposedSize="
                    + proposedSize + ", ETag=" + etag;
    ...
    public class MultipleDeleteResultHandler extends DefaultXmlHandler
    {
        ...
        public void endElement(String name, String elementText)
        {
            ...
            {
                deletedObjectResults.add(result.new DeletedObjectResult(key, version, withDeleteMarker,
                    deleteMarkerVersion));
                inDeleted = false;
